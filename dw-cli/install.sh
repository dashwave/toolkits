...
BINARY_NAME="dw"
BINARY_URL="https://github.com/dashwave/toolkits/releases/download/v0.0.1-alpha/dw"
...
download_dwcli() {
    ...
    say_white "+ Downloading ${BINARY_URL}..."

    # shellcheck disable=SC2046
    # https://github.com/koalaman/shellcheck/wiki/SC2046
    # Disable to allow the `--silent` option to be omitted.
    if curl --retry 3 --fail $(printf %s "${SILENT}") -L "${BINARY_URL}" -o "${TARGET_FILE}"; then
    # if curl -LO https://github.com/dashwave/toolkits/releases/download/v0.0.1-alpha/dw
        chmod +x $TARGET_FILE
    else
        >&2 say_red "error: failed to download ${BINARY_URL}"
